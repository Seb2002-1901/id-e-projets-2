...
> **Kit VERROUILLÉ (source : `pack-production/Assets/tokens/colors.json`) :** Navy `#1B2A4A` · Bleu `#2E5EAA` · **Accent ACTION `#F5A623`** (→ `#FF7A00`) · **Succès `#2EC27E`** · Texte `#222831` · Gris inactif/manqué `#8A93A2` · Bordure `#E6E9EF` · Fond `#F7F8FA` · Surface `#FFFFFF`. Titres **Poppins**, corps **Inter**. Rayons : carte **20**, bouton **14**, pill **999**.
...
> Le raccourci n°1. Un HTML de tout le PDF est prévu à `livraison-ultime/pdf/cap365-pdf.html` (à générer/ouvrir dans le navigateur si présent). La mise en page y est déjà faite : **ouvrir → cadrer chaque page → capturer (pleine résolution) → importer la capture dans le cadre A4 Canva → recadrer proprement**. Canva ne sert alors qu'à la finition + l'export.
>
...
>
> **Aperçus complémentaires à screenshoter comme base directe :** `apercu/screens.html` (8 écrans produit) et `apercu/sales.html`, plus les rendus PNG déjà prêts dans `pack-production/Exports/ecrans/` (ex. `ecran-radar.png`, `ecran-calendrier.png`, `ecran-wrapped.png`).
...
**Canva Pro → « Kit de marque » → Nouveau kit « Cairn — Cap365 ».** Coller les HEX un par un (source exacte : `pack-production/Assets/tokens/colors.json`).
...
- Logos : `pack-production/Assets/svg/logo/` + rendus `pack-production/Exports/svg/logo/`
- Icônes (8 stats + actions) : `pack-production/Assets/svg/icones/` + `pack-production/Exports/svg/icones/`
- Badges : `pack-production/Assets/svg/badges/` + `pack-production/Exports/svg/badges/`
- Composants : `pack-production/Assets/svg/composants/` + `pack-production/Exports/svg/composants/`
- Fonds dégradés (raster prêts) : `pack-production/Exports/fonds/` (`fond-marque-1920x1080.png`, `fond-wrapped-1080x1920.png`, `fond-xp-1080x400.png`)
...
| **Bouton primaire** | Rectangle rayon **14**, remplissage **dégradé XP** `#F5A623→#FF7A00`, ombre `0 2px 10px rgba(245,166,35,.35)`, texte Poppins Bold blanc centré. Base SVG : `pack-production/Assets/svg/composants/bouton-primaire.svg`. |
...
- **SVG :** `pack-production/Assets/svg/logo/cap365-logo-horizontal.svg` · illustration `pack-production/Assets/svg/icones/cairn.svg`
- **PNG :** `pack-production/Exports/svg/logo/cap365-logo-horizontal.png` · fond `pack-production/Exports/fonds/fond-marque-1920x1080.png`
...
- **SVG :** gabarit `pack-production/Assets/svg/composants/carte-defi.svg` (posé comme exemple annoté) · icônes `.../icones/xp-etoile.svg`, `.../icones/serie-flamme.svg`, `.../icones/valider-check.svg`
...
- **SVG :** `pack-production/Assets/svg/composants/radar-8-stats.svg` · `.../composants/progress-xp.svg` · `.../icones/xp-etoile.svg`, `serie-flamme.svg` · 8 icônes de stat `.../icones/` · badges `.../badges/{commun,rare,epique,legendaire}.svg`
...
> Base visuelle exacte : `pack-production/Assets/svg/composants/carte-defi.svg` (viewBox 340×250). Le reproduire dans Canva puis le **grouper** et le **dupliquer**.
...
- **Textes (source H1 de `defis/mois-XX-*.md`) :**
  ...
  Ajouter la phrase d'intro du mois (1er paragraphe du fichier `defis/mois-XX-*.md`) et le jalon (ex. Mois 6 → « Jour 182 · Médaille de Mi-Parcours » ; Mois 12 → « Jour 365 · Cap365 »).
...
- **Textes :** **100 % issus de `pack-production/Notion/defis-365-import-notion.csv`** (voir astuce d'import §6). Ne rien rédiger.
...
- **Textes (ton coach, source `defis/mois-12` §clôture) :** « **Tu l'as fait.** 365 jours, 365 preuves. » · « Tu n'es plus la personne du Jour 1. » · « **Et maintenant ?** Continue le cap. » — **aucune promesse interdite.**
...
- **Textes (source `defis/mois-12` J365) :** « **CERTIFICAT DE TRANSFORMATION** » · « Décerné à ________ » · « Pour avoir accompli les 365 défis de Cap365 et être devenu·e quelqu'un qui tient parole envers soi-même. » · « Fait le ______ · Rang atteint : ______ · Signature : ______ ». Mention « Cap365 · une expérience Cairn ».
...
> Le fichier `pack-production/Notion/defis-365-import-notion.csv` contient déjà **tout le texte des 365 défis** (colonnes : `Jour, Mois, Titre, Catégorie, Stat, Difficulté, Temps, Coût, Objectif, Pourquoi, Instructions, Variante facile, Variante difficile, Récompense`). **Zéro rédaction.**
