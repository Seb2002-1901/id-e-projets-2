...
**Source :** le fichier **déjà fourni** `../CSV/defis-365-import-notion.csv` (365 lignes prêtes à l'import). Chaque défi y est déjà normalisé (Catégorie, Stat, Difficulté, XP, Stat +N, Temps, Coût, Objectif, Pourquoi, Instructions, variantes). *Rappel : le Builder (`../Builder/`) importe ce CSV automatiquement — cette section décrit l'import manuel équivalent.*
...
1. Utilise le fichier fourni `../CSV/defis-365-import-notion.csv` (déjà en **CSV UTF-8**, 365 lignes) — aucune extraction manuelle n'est nécessaire.
